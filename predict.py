...
from visualize import visualize_heads,visualize_segmentation,get_local
# from visualize import get_local
...
def test_softmax(
        test_loader,
        model,
        dataname = '/mnt/c501data/wangcailian/M2FTrans-main/BraTS/BraTS2020_Training_Data',
        feature_mask=None,
        mask_name=None):
    ...
    sensitivity_values = []  # 用于存储每个类别的敏感性值
    one_tensor = torch.ones(1, patch_size, patch_size, patch_size).float().cuda()

    if dataname in ['/SCATrans/BraTS/BRATS2021_Training_Data','/SCATrans/BraTS/BraTS2020_Training_Data', '/SCATrans/BraTS/MICCAI_BraTS_2018_Data_Training']:
        num_cls = 4
...
def test_dice_hd95_softmax(
        test_loader,
        model,
        dataname = '/SCATrans/BraTS/BRATS2021_Training_Data',
        feature_mask=None,
        mask_name=None,
        csv_name=None,
        ):
...
def test_softmax_visualize(
        test_loader,
        model,
        dataname = '/SCATrans/BraTS/MICCAI_BraTS_2018_Data_Training',
        feature_mask=None,
        mask_name=None,
        writer = None,
        step = 0):
    ...
    if dataname in ['/SCATrans/BraTS/BraTS2020_Training_Data', '/SCATrans/BraTS/MICCAI_BraTS_2018_Data_Training']:
        num_cls = 4
        class_evaluation= 'whole', 'core', 'enhancing', 'enhancing_postpro'
        class_separate = 'ncr_net', 'edema', 'enhancing'
    ...
    for i, data in enumerate(test_loader):
        ...
        for h in h_idx_list:
            for w in w_idx_list:
                for z in z_idx_list:
                    ...
                    logging.info(f'Number of attention maps:{len(attention_maps)}')

                    # for i in range(0, len(attention_maps)):
                    #     visualize_heads(writer,attention_maps[i], cols=5, step=global_step, num=i)
                    for i in range(len(attention_maps)):
                        visualize_heads(writer, attention_maps[i], cols=5, step=global_step, num=i,region_type='region_type',output_dir='attention_maps')
                        logging.info(attention_maps[i].max())
                        logging.info(attention_maps[i].min())
                    global_step = global_step + 1
                    logging.info(len(attention_maps))
                    logging.info(attention_maps[0].shape)
                    logging.info(h)
                    logging.info(w)
                    logging.info(z)
        ...
        visualize_segmentation(pred,'/SCATrans/results',global_step)
