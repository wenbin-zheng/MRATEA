...
class general_conv3d(nn.Module):
    def __init__(self, in_ch, out_ch, dilation, k_size=3, stride=1, padding=1, pad_type='reflect', norm='in',
                 act_type='lrelu', relufactor=0.2):
        super(general_conv3d, self).__init__()

        self.conv1 = nn.Conv3d(in_channels=in_ch, out_channels=out_ch, kernel_size=k_size, stride=stride,
                               padding=padding, dilation=dilation, padding_mode=pad_type, bias=True)
        self.norm = normalization(out_ch, norm=norm)
        if act_type == 'relu':
            self.activation = nn.ReLU(inplace=True)
        elif act_type == 'lrelu':
