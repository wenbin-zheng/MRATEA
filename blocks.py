...
from mask import mask_gen_cross4
...
class MBConvBlock(nn.Module):
    def __init__(self, embed_dims, expand_ratio=4, drop_rate=0.):
        super(MBConvBlock, self).__init__()
        self.depthwise_conv = nn.Conv1d(embed_dims, embed_dims * expand_ratio, kernel_size=3, stride=1, padding=1,
                                        groups=embed_dims)
        self.pointwise_conv = nn.Conv1d(embed_dims * expand_ratio, embed_dims, kernel_size=1)
...
class MultiMaskCrossBlock(nn.Module):
 ...
 class MultiMaskAttentionLayer(nn.Module):
     ...
     def forward(self, query, key, value, mask):
         ...
         attn = torch.zeros(qb, qc, qd, qd).to(query.device)
         for rate in self.dilation_rates:
             attn += torch.einsum('bqd, bkd -> bqk', query, key) * (query.shape[-1]) ** -0.5
             self_mask = mask_gen_cross4(qb, qc, qc, mask).cuda(non_blocking=True)
