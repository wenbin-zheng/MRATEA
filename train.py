...
import Parser
import criterions
from lr_scheduler import LR_Scheduler, MultiEpochsDataLoader
from parser import setup
from visualize import visualize_heads, visualize_segmentation
...
parser.add_argument('--dataname', default='/SCATrans/BraTS/MICCAI_BraTS_2018_Data_Training', type=str)
parser.add_argument('--datapath', default='/SCATrans/BraTS/BRATS2018_Training_none_npy', type=str)
parser.add_argument('--savepath', default='/SCATrans/model/brats128/', type=str)
...
setup(args, 'training')
args.train_transforms = ('Compose([RandCrop3D((80,80,80)), '
                         'RandomRotion(10), RandomIntensityChange((0.1,0.1)), RandomFlip(0), NumpyType((np.float32, np.int64)),])')
...
def main():
    ...
    ##########setting models
    if args.dataname in ['/SCATrans/BraTS/BRATS2021_Training_Data', '/SCATrans/BraTS/BraTS2020_Training_Data', '/SCATrans/BraTS/MICCAI_BraTS_2018_Data_Training']:
        num_cls = 4
    else:
        print ('dataset is error')
        exit(0)

    if args.model == 'SCATrans':
    ...
    if args.dataname == '/SCATrans/BraTS/BraTS2020_Training_Data':
        ...
        # valid_file = '/SCATrans/BraTS/BRATS2020_Training_none_npy/val.txt'
        ...
        # valid_file = '/SCATrans/BraTS/BRATS2021_Training_none_npy/val.txt'
        ...
        # valid_file = '/BraTS/BRATS2018_Training_none_npy/val1.txt'
    ...
    for epoch in range(args.num_epochs):
        ...
        #                 file_name)
        #
        #         for z, _ in enumerate(masks_valid_array): writer_visualize = SummaryWriter(log_dir="visualize/result")
        #     visualize_step = 0
    ...
    writer_visualize = SummaryWriter(log_dir="/SCATrans/visualize/result")
    visualize_step = 0
    ...
    with (torch.no_grad()):
        ...
        for i, mask in enumerate(masks_test[::-1]):
            ...
            #     for data in batch:
            #         original_image = data['image'] .cuda()
            #
            #         predicted_mask = model(original_image).argmax(dim=1).cuda()
            #         true_mask = data['mask'].cuda()
            #         visualize_segmentation(original_image[0], predicted_mask[0], true_mask[0],
            #                                f"Testing Mask {mask_name[::-1][i]}", writer_visualize, i)
            #         break  # 处理完一个数据项后退出内部循环
            #     break  # 处理完一个批次后退出外部循环

        logging.info('Avg Dice scores: {}'.format(test_dice_score.avg))
        logging.info('Avg HD95 scores: {}'.format(test_hd95_score.avg))
    writer_visualize.close()
